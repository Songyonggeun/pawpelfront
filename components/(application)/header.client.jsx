            ...
            userRoles.length === 0 ? null : (
              <div className="flex items-center space-x-3">
                {userRoles.includes('ADMIN') ? (
                  <Link href="/admin" className="text-left p-1 hover:text-blue-500 text-sm">관리자페이지</Link>
                ) : (
                  <Link href="/myPage" className="text-left p-1 hover:text-blue-500 text-sm">마이페이지</Link>
                )}
                <button onClick={handleLogout} className="text-left p-1 hover:text-blue-500 text-sm">로그아웃</button>
              </div>
            )
          ) : (
            <div className="p-1 rounded hover:bg-gray-100 text-sm">
              <Link href="/login" className="text-left p-1 hover:text-blue-500 text-sm">로그인</Link>
            </div>
          )}
        </div>
